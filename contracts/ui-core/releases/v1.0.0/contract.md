...
Status: **RELEASE v1.0.0 — NORMATIVE AND CONTENT-FROZEN BY UIC-08**.
...
~~~text
mouse-ui adapter proof:
7d1f0246ca4b70a2a7134de040b02db13e3bd540

mouse-core conformance proof:
f5b7384156a42543505b93dd7609e715c4cd3542

neutral contract candidate proven by both sides:
af7d037646cd0e2917c81eee6acd1f0eb6592921
~~~

This document is the normative language-neutral source of truth for UI↔Core Contract
v1.0.0. The released C binding is `uic_v1.h`.
...
The released contract descriptor contains:
...
The released C binding represents these as unsigned 64-bit values:
...
The released C binding bounds them for safe crossing:
...
The released API is not reentrant for one provider instance.
...
## 19. Released C call surface

The released binding exposes the semantic operations:
...
Normative machine-readable release schema:
...
UIC-06 proved the consumer/UI adapter against this boundary.

UIC-07 proved a Core-side semantic provider against this boundary without transport
leakage.

No MUST-level semantic decision remains open for v1.0.0.

## 22. Immutability

The contents of `contracts/ui-core/releases/v1.0.0/` are content-addressed by
`manifest.json`.

After UIC-08 acceptance and promotion, these normative artifacts MUST NOT be edited in
place. Any semantic/API change requires a new release according to the versioning policy.
